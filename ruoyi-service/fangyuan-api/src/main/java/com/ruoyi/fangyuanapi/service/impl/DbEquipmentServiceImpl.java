...
public class DbEquipmentServiceImpl implements IDbEquipmentService {
     ...
     * */
    @Override
    public int qrCodeGenerate(DbEquipment equipment) throws Exception {
        DbQrCode dbQrCode = new DbQrCode();
        dbQrCode.setEquipmentId(equipment.getEquipmentId());
        dbQrCode.setHeartbeatText(equipment.getHeartbeatText());
        dbQrCode.setCreateTime(new Date());
        int i = dbQrCodeMapper.insertDbQrCode(dbQrCode);

        String methods = "/equipment/equipmentTempList";

        String argument = "?qrCodeId=" + dbQrCode.getQrCodeId();
        String text= url+methods+argument;
