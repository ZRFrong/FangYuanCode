 ...
 * @description: 日期工具类
 */
public class DateUtilLong extends org.apache.commons.lang3.time.DateUtils {

    private static final Log logger = LogFactory.getLog(DateUtilLong.class);
    ...
    public static Date concat(String ymd, String hm) {
        if (!StringUtils.isBlank(ymd) && !StringUtils.isBlank(hm)) {
            try {
                ...
                Date date = DateUtilLong.convert(dateString, DateUtilLong.DATE_TIME_FORMAT);
                return date;
